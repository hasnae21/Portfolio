...
const DEFAULT_PASS = 'hasnae2026';

let certEditingId = null;
let certDeleteId = null;
...
function renderCertificatesTable() {
    ...
    if (certs.length === 0) {
        ...
        document.getElementById('certificates-table').style.display = 'none';
        return;
    }
    
    empty.style.display = 'none';
    document.getElementById('certificates-table').style.display = 'table';
    
    tbody.innerHTML = certs.map(cert => `
        <tr>
            <td><img src="${cert.image}" alt="${cert.title}" style="width:60px;height:40px;object-fit:cover;border-radius:4px"></td>
            <td><strong>${escHtml(cert.title)}</strong></td>
            <td>${escHtml(cert.issuer)}</td>
            <td>${escHtml(cert.date)}</td>
            <td><span class="cat-badge ${cert.category}">${cert.category}</span></td>
            <td>
                <div class="row-actions">
                    <button class="btn-edit" onclick="openEditCertificate('${cert.id}')">
                        <i class="fas fa-edit"></i> Edit
                    </button>
                    <button class="btn-del" onclick="openDeleteCertificate('${cert.id}')">
...
function openAddCertificate() {
    ...
    showModal('certificate-form-modal');
}

function openEditCertificate(id) {
    ...
    document.getElementById('cert-category').value = cert.category || 'training';
...
function saveCertificate(e) {
    ...
    const cert = {
        id: certEditingId || 'cert-' + Date.now(),
    ...
    const certs = getCertificates();
    if (certEditingId) {
        const idx = certs.findIndex(c => c.id === certEditingId);
        if (idx !== -1) certs[idx] = cert;
    } else {
        certs.unshift(cert);
    }
    ...
    showToast(certEditingId ? '✅ Certificate updated!' : '✅ Certificate added!', 'success');
    certEditingId = null;
}

function openDeleteCertificate(id) {
    const cert = getCertificates().find(c => c.id === id);
    if (!cert) return;
    
    certDeleteId = id;
    document.getElementById('cert-delete-name').textContent = cert.title;
    showModal('cert-delete-modal');
}

function confirmDeleteCertificate() {
    if (!certDeleteId) return;
    
    const certs = getCertificates().filter(c => c.id !== certDeleteId);
    saveCertificates(certs);
    closeModal('cert-delete-modal');
    showToast('️ Certificate deleted', 'error');
    certDeleteId = null;
}
...
function renderTable() {
    ...
    tbody.innerHTML = filtered.map(p => `<tr>
        ...
        <td><span class="cat-badge ${p.category}">${CAT_LABELS[p.category] || p.category}</span></td>
        <td>
            <div class="row-tags">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Certificates button */
    document.getElementById('btn-certificates')?.addEventListener('click', () => {
        renderCertificatesTable();
        showModal('certificates-modal');
    });
